(() => {
  if (window["CSScmds"]) {
    console.error("CSScmds ya está definido");
    return;
  }
  ...
  let css_cmdscalc = ".css-cmdscalc {display: none !important;&.protected{display: block !important;}}";
  ...
  function CrearIFW() {
    reglasCMD["IFW"] = (() => {
      ...
      function ifw({ comando, propiedad_valor, operador, sz, min, max } = {}) {
        ...
        let clase = Math.random()
          .toString(36)
          .replace("0.", `ifw-${operador}-`);
        ...
        function ejecutarComando() {
          if (comando) {
            ...
            function ejecutarBetween() {
              const bet_op = /(<|>)/.source;
              if (
                regexGeneric(`${Npx}${bet_op}x${bet_op}${Npx}`).test(comando)
              ) {
